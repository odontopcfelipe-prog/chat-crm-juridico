...
/**
 * Influenciadores — cadastro de parcerias de marketing.
 *
 * Lista todos os influenciadores cadastrados com filtro por status + busca
 * (nome/@handle/cupom). Botão "+ Novo" abre modal com formulário completo
 * (contato, plataforma, audiência, comissão, cupom, status, notas).
 *
 * Acesso restrito a ADMIN (gated no backend via @Roles + RolesGuard).
 */
import { useCallback, useEffect, useMemo, useState } from 'react';
import {
  Megaphone, Plus, Search, Loader2, Pencil, Trash2,
  Instagram, Music2, Youtube, Globe, Phone, Mail, AtSign,
  Tag, Users as UsersIcon, Pause, CheckCircle2, XCircle,
} from 'lucide-react';
...
interface Influencer {
  ...
  commission_value: string | null; // Decimal vem string
...
interface FormState {
  ...
  email: string;
  platform: Platform | '';
  followers: string; // string no form, converte na hora de salvar
  niche: string;
...
export default function InfluencersPage() {
  ...
  // Debounce da busca pra evitar request a cada tecla
  useEffect(() => {
  ...
  const fetchList = useCallback(async () => {
    ...
    try {
      ...
      const msg = err?.response?.data?.message || 'Erro ao carregar influenciadores';
      showError(msg);
      setItems([]);
    } finally {
      setLoading(false);
  ...
  // ─── Guard de UI: se não-admin caiu aqui por URL, mostra aviso ────────
  if (!perms.isAdmin) {
    return (
      <div className="p-6 max-w-2xl mx-auto">
        <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 text-amber-700 dark:text-amber-300">
          ...
          <p className="text-sm">
            O cadastro de influenciadores é restrito a administradores.
          </p>
        </div>
      </div>
    );
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Parcerias com criadores de conteúdo · {counts.ATIVO} ativo{counts.ATIVO === 1 ? '' : 's'}
              </p>
            </div>
          </div>
          <button
        ...
        {loading ? (
          ...
          <div className="text-center py-16 bg-card border border-dashed border-border rounded-xl">
            ...
            <p className="text-sm text-muted-foreground mb-1">
              {debouncedQuery || statusFilter ? 'Nenhum influenciador encontrado com esses filtros.' : 'Nenhum influenciador cadastrado ainda.'}
            </p>
      ...
      >
        <div className="space-y-4">
          ...
          <fieldset className="space-y-3 pt-3 border-t border-border">
            ...
            <Field label="Cupom de desconto associado (opcional)">
              <input
              ...
              <p className="text-[10px] text-muted-foreground mt-1">
                Útil para rastrear conversões. Único por clínica.
              </p>
            </Field>
          </fieldset>
...
// ─── Subcomponentes locais (mantidos no arquivo pra simplicidade) ─────────
