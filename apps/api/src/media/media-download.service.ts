 ...
 *
 * Baixa midia da Evolution API, salva no filesystem local e cria o record Media,
...
export class MediaDownloadService {
  ...
  /**
   * Baixa midia da Evolution API, salva no filesystem, cria record no Prisma.
  ...
  async downloadAndStore(params: {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        // 4. Salvar no filesystem
        const mimeBase = mimeType.split(';')[0].trim();
        const ext = mimeBase.split('/')[1] || 'bin';
        const relativePath = this.fileStorage.buildRelativePath(messageId, ext);
        ...
        this.logger.log(`[MEDIA-SYNC] Midia baixada e salva: ${relativePath} (${size} bytes)`);
        ...
        this.logger.warn(
          `[MEDIA-SYNC] Tentativa ${attempt}/${MAX_ATTEMPTS} falhou para msg ${messageId}: ${reason}`,
        );
  ...
  private async uploadToDriveIfNeeded(
    ...
    if (!media?.file_path || !media?.mime_type) return;
    ...
    const fileBuffer = await this.fileStorage.getBuffer(media.file_path);
