...
export class PatientTagsService {
  ...
  async setTagsForPatient(patientId: string, tenantId: string, tagIds: string[]) {
    ...
    return this.getTagsForPatient(patientId, tenantId);
  }

  async getTagsForPatient(patientId: string, tenantId: string) {
    // Onda 17.33/17.34 — isolamento multi-tenant: PatientTagOnPatient não tem
    ...
    // sabendo o id (enumeration). tenantId agora é OBRIGATÓRIO — parâmetro
    // opcional era brecha esperando um caller esquecer.
    const patient = await this.prisma.patient.findUnique({
      where: { id: patientId },
      select: { tenant_id: true },
    });
