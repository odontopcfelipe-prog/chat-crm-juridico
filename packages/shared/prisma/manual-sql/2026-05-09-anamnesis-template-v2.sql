-- ============================================================================
-- Migration manual — 2026-05-09 — AnamnesisTemplate V2
-- ============================================================================
-- Cria/atualiza template V2 odontologico:
--   - 9 secoes (vs 6 da V1)
--   - Todos campos opcionais (required: false) — nada bloqueia o save
--   - Foco em Sim/Nao (boolean) + textos abertos opcionais
--   - Cobre: dados gerais, historico medico, alergias, medicamentos, habitos,
--     historico odontologico, saude bucal, gestacao, historico familiar,
--     observacoes livres
--
-- IDEMPOTENTE.
-- ============================================================================
...
BEGIN
    ...
    "sections": [
      {
        ...
        "questions": [
          ...
          { "id": "blood_type", "type": "select", "label": "Tipo sanguineo",
            "options": ["A+","A-","B+","B-","AB+","AB-","O+","O-","Nao sei"], "required": false }
        ]
      },
      {
        "id": "medical_history",
        ...
        "questions": [
          { "id": "chronic_diseases", "type": "multiselect", "label": "Possui alguma doenca cronica?",
            "options": ["Diabetes","Hipertensao","Doenca cardiaca","Doenca renal","Doenca hepatica","Problema de tireoide","Epilepsia","Asma","Anemia","Outra","Nenhuma"],
            "required": false },
      ...
      },
      {
        ...
        "questions": [
          { "id": "smoker", "type": "select", "label": "Fuma?",
            "options": ["Nunca","Ex-fumante","Fumante ocasional","Fumante diario"], "required": false },
          { "id": "alcohol", "type": "select", "label": "Consome alcool?",
            "options": ["Nao","Socialmente","Frequentemente","Diariamente"], "required": false },
      ...
      },
      {
        ...
        "questions": [
          { "id": "last_visit", "type": "select", "label": "Quando foi sua ultima consulta odontologica?",
            "options": ["Menos de 6 meses","6 a 12 meses","1 a 2 anos","Mais de 2 anos","Nunca"], "required": false },
          { "id": "brushing_frequency", "type": "select", "label": "Frequencia de escovacao",
      ...
      },
      {
        ...
        "questions": [
          ...
          { "id": "sensitivity", "type": "multiselect", "label": "Sensibilidade a:",
            "options": ["Frio","Calor","Doces","Pressao","Nenhuma"], "required": false }
        ]
      },
      {
        "id": "pregnancy",
        "title": "Gestacao e Hormonios (se aplicavel)",
        "questions": [
      ...
      },
      {
        ...
        "questions": [
          { "id": "family_diseases", "type": "multiselect", "label": "Parentes proximos com:",
            "options": ["Diabetes","Hipertensao","Cancer","Doenca cardiaca","Problemas dentarios graves","Nenhum"],
            "required": false }
        ]
      },
      {
        "id": "notes",
        "title": "Observacoes Livres",
        "questions": [
  ...
  FOR t IN SELECT id FROM "Tenant" LOOP
    -- Procura template existente com versao >= 2 (idempotencia: se ja rodou antes)
    SELECT id INTO existing_id
    FROM "AnamnesisTemplate"
    WHERE tenant_id = t.id AND version >= 2
    ORDER BY version DESC
    LIMIT 1;

    IF existing_id IS NOT NULL THEN
      UPDATE "AnamnesisTemplate"
      SET schema     = v2_schema,
      ...
      RAISE NOTICE 'Tenant %: template V2 atualizado (id=%)', t.id, existing_id;
    ELSE
